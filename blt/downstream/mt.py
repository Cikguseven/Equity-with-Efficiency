...
def build_fewshot_prompt(
    ...
    if n_shots > 0:
        if src_lang == "en" and tgt_lang in _SHOT_TGT:
            shot_src = _SHOT_EN
            shot_tgt = _SHOT_TGT[tgt_lang]
        elif tgt_lang == "en" and src_lang in _SHOT_TGT:
            shot_src = _SHOT_TGT[src_lang]
            shot_tgt = _SHOT_EN

    parts: list[str] = [f"Translate this from {src_name} to {tgt_name}:\n"]

    if shot_src and shot_tgt:
        for s, t in zip(shot_src[:n_shots], shot_tgt[:n_shots]):
            parts.append(f"{src_name}: {s}\n{tgt_name}: {t}")
        parts.append(f"{src_name}: {test_src}\n{tgt_name}:")
    else:
        parts.append(f"{src_name}: {test_src}\n{tgt_name}:")
...
def _build_unicode_to_byte() -> dict[int, int]:
    bs = (
...
_GPT2_THAI_RE = re.compile(r"à[¸¹º»¼½¾¿]")

def _has_gpt2_byte_chars(text: str) -> bool:
    if any(256 <= ord(c) <= 323 for c in text):
        return True
    if _GPT2_THAI_RE.search(text):
        return True
    return False

def fix_bpe_byte_chars(text: str) -> str:
    if not _has_gpt2_byte_chars(text):
        return text
    ...
    for i, c in enumerate(chars):
        if c == " ":
            prev_surrogate = (
                i > 0
                and ord(chars[i - 1]) > 127
                and ord(chars[i - 1]) in _UNICODE_TO_BYTE
            )
            next_surrogate = (
                i < len(chars) - 1
                and ord(chars[i + 1]) > 127
            ...
            if prev_surrogate and next_surrogate:
                continue
        cleaned.append(c)
    text = "".join(cleaned)
    ...
    for c in text:
        cp = ord(c)
        if cp in _UNICODE_TO_BYTE:
            result_bytes.append(_UNICODE_TO_BYTE[cp])
        else:
    ...
    for trim in range(1, min(4, len(result_bytes) + 1)):
        try:
            return result_bytes[:-trim].decode("utf-8")
        except UnicodeDecodeError:
            continue
...
def _fix_ascii_intra_word_spaces(text: str) -> str:
    import regex
    pattern = regex.compile(r'(?<!\w)([A-Za-z]{1,4}) (?=\w)')
    prev = None
    while prev != text:
        prev = text
...
def clean_generation_text(text) -> str:
    ...
    text = fix_bpe_byte_chars(text)

    if _has_gpt2_byte_chars(text) or "Ġ" in text:
        text = re.sub(r'  +', ' ', text)

    text = text.replace("\r\n", "\n").replace("\r", "\n").strip()
    text = text.replace("\u200b", "").replace("\ufeff", "")
    text = text.strip()
    text = _fix_ascii_intra_word_spaces(text)
    return text.strip()
...
def extract_translation(
    ...
    if out.lower().startswith("translate this from"):
        return ""
...
# ---------------------------------------------------------------------------
# 6. HELPERS: TORCH DTYPE RESOLUTION, MODEL INPUT DEVICE
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. STANDARD HUGGINGFACE MODEL LOADING & INFERENCE
# ---------------------------------------------------------------------------
...
def evaluate_direction(
    ...
    can_fewshot = n_shots > 0 and (
        (src_lang == "en" and tgt_lang in _SHOT_TGT)
        or (tgt_lang == "en" and src_lang in _SHOT_TGT)
    )
    ...
    hyps_raw = [
        ...
        for p, o in zip(bare_prompts, raw_out)
    ]
...
def parse_args():
    ...
    parser.add_argument("--flores_dir", default="/localhome/user/flores-plus_dev_devtest")
    ...
    parser.add_argument(
        ...
        help="Number of few-shot examples (0 = zero-shot). Default: 5",
    )
