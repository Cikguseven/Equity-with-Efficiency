...
if [ "$BYTE_PATCHES" = 0 ] ; then
    ...
    /home/user/project/myte/src/prepare_corpus_lexicon.py "${FILE_NAME}" "${CORPUS_FILE}" --lexicon "${LEXICON_FILE}"
else
    /home/user/miniconda3/envs/project/bin/python \
    /home/user/project/myte/src/prepare_corpus_lexicon.py "${FILE_NAME}" "${CORPUS_FILE}"  --lexicon "${LEXICON_FILE}" --byte_patches --patch_size "${BYTE_PATCHES}"
fi
